...
mod execution {
	...
	where
		...
		H::Out: Ord + 'static + codec::Codec,
	{
		let mut trie_backend = create_proof_check_backend::<H>(root, proof)?;
		...
		I::Item: AsRef<[u8]>,
	{
		...
		prove_child_read_on_trie_backend(&mut trie_backend, child_info, keys)
	}
	...
	where
		...
		I::Item: AsRef<[u8]>,
	{
		let mut proving_backend = create_proof_check_backend::<H>(root, proof)?;
		let mut result = HashMap::new();
		for key in keys.into_iter() {
			let value = read_proof_check_on_proving_backend(&mut proving_backend, key.as_ref())?;
	...
	{
		let mut proving_backend = create_proof_check_backend::<H>(root, proof)?;
		read_range_proof_check_with_child_on_proving_backend(&mut proving_backend, start_at)
	}
	...
	pub fn read_proof_check_on_proving_backend<H>(
		proving_backend: &mut TrieBackend<MemoryDB<H>, H>,
		key: &[u8],
...
mod tests {
	...
	fn execute_works_inner(state_version: StateVersion) {
		let mut backend = trie_backend::tests::test_trie(state_version, None, None);
	...
	fn clear_prefix_in_ext_works() {
		...
		let mut backend = state.as_trie_backend();
	...
	fn limited_child_kill_works() {
		...
		let mut backend = InMemoryBackend::<BlakeTwo256>::from((initial, StateVersion::default()));
	...
	fn append_storage_works() {
		...
		{
			let ext = Ext::new(&mut overlay, &mut backend, None);
			assert_eq!(ext.storage(key.as_slice()), Some(vec![reference_data[0].clone()].encode()));
		}
	}
	...
	fn child_storage_uuid() {
		...
		let mut transaction = {
			let mut backend = test_trie(state_version, None, None);
			let mut ext = Ext::new(&mut overlay, &mut backend, None);
			...
			overlay.drain_storage_changes(&mut backend, state_version).unwrap().transaction
		};
