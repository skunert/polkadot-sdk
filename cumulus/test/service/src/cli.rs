...
impl SubstrateCli for TestCollatorCli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		Ok(match id {
			...
			"elastic-scaling-single-slot" => {
				tracing::info!("Using elastic-scaling single-slot chain spec.");
				Box::new(cumulus_test_service::get_elastic_scaling_single_slot_chain_spec(Some(
					ParaId::from(2300),
				))) as Box<_>
			},
			path => {
